...
export function Navbar() {
  ...
  return (
    <div className="flex items-center justify-between mb-4 border-b border-secondary/30 p-4 w-full relative">
      <button
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        <button
          ...
          className="hover:cursor-pointer relative rounded-full flex items-center bg-tertiary  p-3"
        >
          <ShoppingBag size={16} className="hover:text-secondary" />
          {user && totalItems > 0 && (
            <span className="absolute -top-1 -right-1 min-w-5 h-5 px-1 rounded-full bg-secondary text-white text-[10px] flex items-center justify-center">
              {totalItems}
            </span>
          )}
        </button>
        {!user ? (
          <Link
            href="/login"
            className="hidden sm:block rounded-full bg-tertiary py-2 px-4 md:px-6 font-semibold hover:text-secondary"
          >
            Login
          </Link>
        ) : (
      ...
      {drawerOpen && (
        ...
        >
          ...
          >
            <div className="mb-4 border-b border-secondary/30 pb-4 flex items-center justify-between">
              <Logo />
            </div>
            <nav className="space-y-1">
              <Link
                href="/"
                className="block px-3 py-2 rounded hover:bg-secondary/30"
                onClick={() => setDrawerOpen(false)}
              >
                Home
              </Link>
            ...
            {!user && (
              <div className="mt-4">
                <Link
                  href="/login"
                  onClick={() => setDrawerOpen(false)}
                  className="block text-center rounded-full bg-tertiary border border-secondary text-white font-semibold py-2"
                >
                  Sign In
                </Link>
              </div>
